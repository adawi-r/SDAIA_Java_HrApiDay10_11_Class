...
public class EmployeeDAO {
    ...
    private static final String SELECT_EMP_WITH_LAST_NAME = "select * from employees where last_name = ?";
    private static final String SELECT_ONE_DEPT_JOIN_LOC = "select * from employees join jobs on employees.job_id = jobs.job_id where job_id = ?";
...
//        st.setInt(9, e.getDepartment_id());
    ...
    public ArrayList<Employee> selectAllDEP(EmployeeFilterDto filter) throws SQLException, ClassNotFoundException {
        Class.forName("org.sqlite.JDBC");
        Connection conn = DriverManager.getConnection(URL);
    ...
    public Employee selectEMPByLastName(String last_name) throws SQLException, ClassNotFoundException {
        Class.forName("org.sqlite.JDBC");
        Connection conn = DriverManager.getConnection(URL);
        PreparedStatement st = conn.prepareStatement(SELECT_EMP_WITH_LAST_NAME);
